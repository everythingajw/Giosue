...
if generate_statement:
    ...
    statement_visitor_interface_methods = [
        f"{GENERIC_PARAMETER} Visit{tree.name}{tree.base_class_name}({tree.name} expression);"
    ...
    statement_base_class_methods = [
        f"public abstract {GENERIC_PARAMETER} Accept<{GENERIC_PARAMETER}>({VISITOR_INTERFACE_NAME} visitor);"
    ]

    statement_base_class = add_namespace([
        f"public abstract class {BASE_STATEMENT_CLASS_NAME}",
        "{",
        *indent(statement_base_class_methods),
        "}"
    ], statement_tree_namespace)

    # Write the statement files
    with open(statement_output_dir / "IVisitor.cs", "w") as f:
        f.writelines("\n".join(using_statements))
        f.write("\n\n")
        f.write("\n".join(statement_visitor_interface))

    with open(statement_output_dir / f"{BASE_STATEMENT_CLASS_NAME}.cs", "w") as f:
        f.writelines("\n".join(using_statements))
        f.write("\n\n")
        f.write("\n".join(statement_base_class))

    for tree in statement_trees:
        output_file_path = statement_output_dir / f"{tree.name}.cs"
        with open(output_file_path, "w") as f:
            f.writelines("\n".join(using_statements))
            f.write("\n\n")
            f.write(tree.generate_tree())
